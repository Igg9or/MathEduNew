...
def import_templates_from_json(conn, json_text):
    ...
    for tpl in templates:
        ...
        question = tpl.get('question_template', '')

        # Если указано фото — очищаем текст задания (как в lesson_tasks)
        if photo:
            question = ''

        # ВАЖНО: если в JSON нет photo_url, НЕ затираем существующее фото в БД.
        # Обновляем photo_path только если оно явно передано в JSON.
        if photo:
            # Полное обновление включая фото
            cursor.execute("""
            ...
            # Обновление БЕЗ затираания photo_path (сохраняем ранее загруженные фото)
            cursor.execute("""
                INSERT INTO task_templates
                (textbook_id, name, question_template, answer_template,
                 parameters, answer_type, conditions, photo_path)
    ...
    conn.commit()
    imported_names = [tpl['name'] for tpl in templates]
    return True, f"Импортировано шаблонов: {imported}", imported_names
